...
function import_get_entry_date($entryDate)
{
    $parts = explode(' ', $entryDate, 3);
    $cleanedDateString = $parts[0] . ' ' . $parts[1];

    return $time_input = strtotime($cleanedDateString);

    return date('d/m/Y h:i:s', $time_input);
}
...
function import_get_expiration_date($start_expiration, $expiration, $fechaIngreso = null)
{
    ...
    $fechaIngreso = import_get_entry_date($fechaIngreso);

    if ($fechaIngreso) {
        $fecha_caducidad = strtotime("+" . $days_expiration . " days", $fechaIngreso);
    } else {
...
function import_process_client_registration($sku, $order, $entryDate = null, $manual = false)
{
    ...
    $date_of_entry = (isset($entryDate)) ? import_get_entry_date($entryDate) : strtotime('now');
