<?php
/**
 * Template part for displaying posts from the seventh category for magazine page.
 *
 * @package Melina
 */
		...
		<div class="container">
				...
				while ( $query_category_posts->have_posts() ) : $query_category_posts->the_post();
					...
					if ( $i % 5 == 0 ) : ?>
						...
						<article class="post-card post-card--wide<?php if ( has_post_thumbnail() && ! post_password_required() ) : ?> post-card--has-thumbnail<?php endif; ?>">
							<?php melina_post_thumbnail( 'post-thumbnail', 'post-card__thumbnail', array( 'sizes' => '(max-width: 479px) 90vw, (max-width: 599px) 432px, (max-width: 767px) 536px, (max-width: 959px) 688px, 768px' ), true ); ?>
	        ...
	        elseif ( $i % 5 == 1 ) : ?>
	          ...
	          <article class="post-card post-card--tall<?php if ( has_post_thumbnail() && ! post_password_required() ) : ?> post-card--has-thumbnail<?php endif; ?>">
	            <?php melina_post_thumbnail( 'post-thumbnail', 'post-card__thumbnail', array( 'sizes' => '(max-width: 479px) 90vw, (max-width: 599px) 432px, (max-width: 767px) 536px, (max-width: 959px) 328px, (max-width: 1023px) 368px, 711px' ), true ); ?>
